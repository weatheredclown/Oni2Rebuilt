...
pub fn decode_tex(data: &[u8]) -> Option<(u32, u32, Vec<u8>)> {
...
pub fn load_tga_file(dir: &str, filename: &str, images: &mut ResMut<Assets<Image>>) -> Option<Handle<Image>> {
...
pub fn load_tga_texture(
    ...
    if crate::vfs::exists(entity_dir, &tex_filename) {
        if let Ok(tex_bytes) = crate::vfs::read(entity_dir, &tex_filename) {
            if let Some((width, height, rgba)) = decode_tex(&tex_bytes) {
                info!("Loaded native .tex texture: {:?} ({}x{})", tex_filename, width, height);
                let mut image = Image::new(
                    bevy::render::render_resource::Extent3d {
                        width,
                        height,
    ...
    if crate::vfs::exists(entity_dir, &tga_filename) {
        if let Some(handle) = load_tga_file(entity_dir, &tga_filename, images) {
            info!("Loaded .tex.tga texture: {:?}", tga_filename);
            return Some(handle);
        }
    }
