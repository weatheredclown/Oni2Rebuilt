...
pub fn decode_tex(data: &[u8]) -> Option<(u32, u32, Vec<u8>, bool)> {
    ...
    match tex_type {
        0x01 | 0x0E => {
            ...
            if data.len() < 0x40E + (width * height) as usize {
                return None;
            }
            let clut_off = 0x0E;
            let data_off = 0x40E;
        ...
        0x0F | 0x10 => {
            ...
            if data.len() < 0x4E + (width * height / 2) as usize {
                return None;
            }
            let clut_off = 0x0E;
            let data_off = 0x4E;
        ...
        0x11 => {
            // RGB888
            let data_off = 0x0E;
            if data.len() < data_off + (width * height * 3) as usize {
                return None;
            }
            for y in 0..height {
                for x in 0..width {
        ...
        0x12 => {
            // RGBA8888
            let data_off = 0x0E;
            if data.len() < data_off + (width * height * 4) as usize {
                return None;
            }
            for y in 0..height {
                for x in 0..width {
        ...
        0x06 => {
            // RGBA4444
            let data_off = 0x0E;
            if data.len() < data_off + (width * height * 2) as usize {
                return None;
            }
            for y in 0..height {
                for x in 0..width {
