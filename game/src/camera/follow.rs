...
pub fn follow_camera_system(
    ...
    for (controller, mut channel) in &mut camera_query {
        if controller.active_mode != ActiveCameraMode::GameNavigation && controller.active_mode != ActiveCameraMode::GameTargeting {
            continue;
        }
        ...
        if !scroll_active {
            ...
            channel.desired_distance += (channel.package_distance - channel.desired_distance) * recovery_speed;
        }
