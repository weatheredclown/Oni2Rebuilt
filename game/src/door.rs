...
fn register_door_system(
    ...
    query: Query<(Entity, &Oni2Entity, &Transform), (Without<RegisteredDoor>, Without<DoorComponent>)>,
    ...
    for (entity, oni_ent, tf) in &query {
        ...
        if name_lower.contains("actorspecific") || name_lower.contains("door") {
            ...
            info!("Registered door {} with nav graph door index {}", oni_ent.name, door_idx);
        }
    }
}
        ...
        if let Ok((mut opt_door, opt_lib, opt_state)) = q_doors.get_mut(ev.target) {
            ...
            if let (Some(lib), Some(mut state)) = (opt_lib, opt_state) {
                if lib.play(anim_name, &mut state) {
                    ...
                    warn!("Failed to play animation {} on door {:?}", anim_name, ev.target);
                }
            } else {
                warn!("Door {:?} missing animator, skipping visual sequence", ev.target);
            }
            ...
            info!("Action processed on Door {:?}! Current state: {}", ev.target, if is_now_open { "OPEN" } else { "CLOSED" });
        }
    } else {
