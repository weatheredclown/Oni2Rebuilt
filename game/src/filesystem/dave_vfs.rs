    ...
    pub fn new(archive_path: &str) -> std::io::Result<Self> {
        ...
        if !resolved_path.exists() {
            return Err(io::Error::new(
                ...
                format!("Archive not found at {:?}", resolved_path),
            ));
        }
        ...
        info!(
            "Mounted Dave VFS: {} files, {} directories",
            files.len(),
            directories.len()
        );

        Ok(Self {
            archive_path: resolved_path.to_str().unwrap_or(archive_path).to_string(),
            files,
            directories,
        })
    }
...
impl Vfs for DaveVfs {
    fn read(&self, dir: &str, filename: &str) -> io::Result<Vec<u8>> {
        ...
        if let Some(entry) = self.files.get(&path_key) {
            ...
            warn!(
                "DaveVfs Read (MISS): {} -> File not found in archive",
                path_key
            );
            Err(io::Error::new(
                io::ErrorKind::NotFound,
                "File not found in Dave VFS",
            ))
        }
    }
    ...
    fn read_dir(&self, path: &str) -> io::Result<Vec<VfsEntry>> {
        let normalized = path.replace('\\', "/").to_lowercase();
        let search = if normalized.starts_with('/') {
            normalized[1..].to_string()
        } else {
            normalized
        };
        ...
        warn!(
            "DaveVfs ReadDir (MISS): {} -> Directory not found in archive",
            search
        );
        Err(io::Error::new(
            io::ErrorKind::NotFound,
            "Directory not found in Dave VFS",
        ))
    }
