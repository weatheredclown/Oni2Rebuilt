...
use crate::statemachine::drivers::attack::{AttackDriver, ATTACK_ACTION_PARSER, ATTACK_EVENT_PARSER};
use crate::statemachine::drivers::fight::{FightDriver, FIGHT_ACTION_PARSER, FIGHT_EVENT_PARSER};
use crate::statemachine::drivers::parse::parse_sm;
use crate::statemachine::drivers::squad::{SquadDriver, SQUAD_ACTION_PARSER, SQUAD_EVENT_PARSER};
    ...
    pub fn get_or_load(&mut self, asset_base: &str) -> Option<Arc<SmData<FightDriver>>> {
        ...
        match crate::vfs::read_to_string(path, filename) {
            ...
            Err(e) => {
                ...
                bevy::log::warn!("fightai: Failed to load {} from VFS (trying fallback... {})", filename, e);
                ...
                match std::fs::read_to_string(&fb_path) {
                    Ok(content) => {
                        match parse_sm::<FightDriver>(&content, FIGHT_EVENT_PARSER, FIGHT_ACTION_PARSER) {
                            Ok(sm_data) => {
                                bevy::log::info!("fightai: Successfully loaded {} (fallback)", filename);
                                let arc = Arc::new(sm_data);
                                self.data = Some(Arc::clone(&arc));
                                Some(arc)
                            }
                            Err(e) => {
                                bevy::log::error!("fightai: Failed to parse fallback {}: {}", filename, e);
                                None
                            }
                        }
                    }
...
impl SquadFsmCache {
    ...
    pub fn get_or_load(&mut self) -> Option<Arc<SmData<SquadDriver>>> {
        ...
        match crate::vfs::read_to_string("statemachine", filename) {
            Ok(content) => match parse_sm::<SquadDriver>(&content, SQUAD_EVENT_PARSER, SQUAD_ACTION_PARSER) {
                Ok(sm_data) => {
                    bevy::log::info!("fightai: Successfully loaded {}", filename);
                    let arc = Arc::new(sm_data);
                    self.data = Some(Arc::clone(&arc));
...
impl AttackFsmCache {
    ...
    pub fn get_or_load(&mut self, table_name: &str, asset_base: &str) -> Option<Arc<SmData<AttackDriver>>> {
        ...
        match parse_sm::<AttackDriver>(&content, ATTACK_EVENT_PARSER, ATTACK_ACTION_PARSER) {
            Ok(sm_data) => {
                bevy::log::info!("fightai: Successfully loaded {}", filename);
                let arc = Arc::new(sm_data);
