...
pub const ALTERNATE_SCAN: [usize; 64] = [
...
pub fn inverse_scan(coefficients: &[i16; 64], alternate: bool) -> [i16; 64] {
    ...
    let mut out = [0; 64];
    for (idx, &dest) in scan.iter().enumerate() {
        out[dest] = coefficients[idx];
    }
    out
}
